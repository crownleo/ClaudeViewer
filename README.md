...
> 演进脉络：v1 对话查看与虚拟滚动 → v2 ZIP 导入与多类型数据 → v3 全局搜索与统计 → v4 搜索侧边栏、热力图、本地持久化、LaTeX、混合渲染 → v5 稳定整合发布 → v5.1 一键复制与间距优化 → v5.2 舍弃 CDN、依赖内联 → v5.3 个人记忆、工具调用、对话导航 → v5.4 移动端适配 → v5.5 MD 导出修复、统计图美化。
