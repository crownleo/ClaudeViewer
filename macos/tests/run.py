...
without_policy = re.sub(r'<meta\s+http-equiv="Content-Security-Policy"\s+content="[^"]*"\s*/?>', '', original, flags=re.IGNORECASE)
assert restored == without_policy, "CSP transformation must not patch application logic"
assert len(re.findall(r'<meta\s+http-equiv="Content-Security-Policy"', bundled, flags=re.IGNORECASE)) == 1
adapted = build.bundled_html(original)
assert "Optional Mac native storage adapter" in adapted
assert "connect-src claude-archive:" in adapted
assert len(re.findall(r'<meta\s+http-equiv="Content-Security-Policy"', adapted, flags=re.IGNORECASE)) == 1
try:
    build.bundled_html(original.replace("const APP_VERSION='v6.0';", "const APP_VERSION='v6.1';"))
except ValueError:
    pass
else:
    raise AssertionError("An unverified upstream version must not be silently packaged")
assert (root / "claude_viewer.html").read_text(encoding="utf-8") == original
print("Native bundle: one nonce CSP, pinned upstream, isolated adapter, unchanged source passed.")
